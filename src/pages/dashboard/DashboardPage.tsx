...
export default function DashboardPage() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      setStats({
        ...
        bestSelling: bestSellingData?.map(product => ({
          name: product.title,
  ...
  const dashboardOrderColumns = [
    { header: 'Item', accessor: (row: any) => <div className="text-sm font-medium text-gray-900">{row.item}</div> },
    { header: 'Date', accessor: (row: any) => <div className="text-sm text-gray-500">{row.date}</div> },
    { header: 'Total', accessor: (row: any) => <div className="text-sm text-gray-500">${row.total.toFixed(2)}</div> },
    ...
    <>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        ...
        <div className="bg-white p-6 border border-gray-200 rounded-lg overflow-hidden">
          <div className="flex justify-between mb-4">
            ...
            <p className="text-2xl font-semibold mt-2">{stats.customers.toLocaleString()}</p>
          </div>
          <div className="h-34">
        ...
        <div className="bg-white p-6 border border-gray-200 rounded-lg overflow-hidden flex flex-col justify-between">
          <div className="flex justify-between mb-4">
            <div>
              ...
              <p className="text-gray-400 text-xs mt-1">MONTHLY GOALS: {monthlyGoal.toLocaleString()}</p>
            </div>
            <p className="text-2xl font-semibold mt-2">{stats.orders.toLocaleString()}</p>
          </div>
          <div className="">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-1 bg-white border border-gray-200 rounded-lg overflow-hidden">
          ...
          <div className="p-6">
            <p className="text-2xl font-semibold mt-2">$2,400 - <span className="text-gray-500 text-sm">Total Sales</span></p>
          </div>
