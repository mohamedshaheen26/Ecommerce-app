  ...
  const fetchOrders = async () => {
    try {
      ...
      const ordersWithUserInfo = await Promise.all(
        ((ordersData || []) as IOrderResponse[]).map(async (order) => {
          ...
          const typedUserData = userData as IUser;
          return {
            ...order,
      ...
      const completeOrders = await Promise.all(
        ordersWithUserInfo.map(async (order) => {
          ...
          return {
            ...order,
            order_items: (
              (itemsData || []) as unknown as IOrderItemResponse[]
            ).map((item) => ({
              id: item.id,
    ...
    },
    {
      ...
      accessor: (order: IOrder) => (
        <select
          value={order.status}
          onChange={(e) =>
            handleStatusChange(order.id, e.target.value as IOrder["status"])
          }
